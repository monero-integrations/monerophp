...
class daemonRPC
{
	 ...
	 *
	 */
	public function on_getblockhash(int $height)
	{
		$params = [$height];
	 ...
	 *
	 */
	public function getblocktemplate(string $wallet_address, int $reserve_size)
	{
	...
	*/
	...
	{
		$params = ['tx_as_hex' => $tx_as_hex, 'do_not_relay' => $do_not_relay, 'do_sanity_checks' => $do_sanity_checks];
	...
	public function start_mining($background_mining, $ignore_battery = false, $miner_address, $threads_count = 1)
	{
		...
		$params = ['do_background_mining' => $background_mining, 'ignore_battery' => $ignore_battery, 'miner_address' => $miner_address, 'threads_count' => $threads_count];
